...
type Role = 'billing' | 'approver' | 'accounts' | 'admin';
...
export default function LoginPage() {
    ...
    const checkSession = async () => {
      try {
        ...
        if (res.ok && data.authenticated) {
          ...
          if (data.role === 'accounts') { router.push('/accounts/dashboard'); return; }
  ...
  const handleAccountsLogin = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/auth/accounts-login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: accountsEmail.trim(), password: accountsPassword }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || 'Invalid credentials');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    else if (activeRole === 'accounts') handleAccountsLogin();
      ...
      <div className="w-full max-w-md relative z-10 fade-in">
        ...
        <div className="glass-card p-7">
          ...
          <div className="role-tabs mb-6">
            ...
            <button
              type="button"
              className={`role-tab ${activeRole === 'accounts' ? 'active' : ''}`}
              onClick={() => { setActiveRole('accounts'); setError(''); }}
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
