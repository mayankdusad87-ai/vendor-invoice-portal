                ...
                >
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      // Fetch approval history when expanding submitted/approved/payment-phase invoices
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 pt-0">
                      <div className="border-t border-gray-100 pt-4">
                          ...
                          <div className="mb-4 rounded-lg p-4 bg-emerald-50/50 border border-emerald-100">
                            <p className="text-sm font-semibold text-gray-900 mb-2 flex items-center gap-2">
                              ...
                              {invoice.taxInvoiceNumber && (
                                <span className="text-xs text-gray-500 font-normal">#{invoice.taxInvoiceNumber}</span>
                              )}
                            </p>
                                ...
                                <>
                                  {taxIsImage && (
                                    <img src={invoice.taxInvoiceFileUrl} alt={`Tax Invoice ${invoice.taxInvoiceNumber || ''}`}
                                      className="w-full max-h-[500px] object-contain rounded-lg cursor-pointer hover:opacity-90 transition-opacity bg-white border border-gray-200"
                                      onClick={() => setLightboxUrl(invoice.taxInvoiceFileUrl!)} />
                                  )}
                                  {taxPreview && (
                                    <iframe src={taxPreview} className="w-full rounded-lg border border-gray-200"
                                      style={{ height: '500px' }} title={`Tax Invoice ${invoice.taxInvoiceNumber || ''} preview`} allow="autoplay" />
                                  )}
                        ...
                        {/* Revision info — base/GST changed from proforma to tax invoice */}
                        ...
                        {(invoice.physicalCopySentAt || invoice.physicalCopyReceivedAt) && (
                        ...
                        {needsExtension(invoice) && (() => {
                          ...
                          const origBase = invoice.originalAmount ? parseFloat(invoice.originalAmount) || 0 : 0;
                          const origGst = invoice.originalGstAmount ? parseFloat(invoice.originalGstAmount) || 0 : 0;
                          const hasBaseRevision = origBase > 0 && Math.abs(origBase - baseAmt) > 0.01;
                          const hasGstRevision = origGst > 0 && Math.abs(origGst - gst) > 0.01;
                          return (
                            <div className="mb-4 rounded-lg border-2 border-pink-200 bg-pink-50/50 p-4">
                              ...
                              {/* Before / After comparison if revision happened */}
                              {(hasBaseRevision || hasGstRevision) && (
                                <div className="mb-3 bg-white rounded-lg p-3 border border-amber-200">
                                  <p className="text-[10px] font-semibold text-amber-700 uppercase mb-2">Proforma → Tax Invoice Changes</p>
                                  <div className="grid grid-cols-2 gap-2 text-xs">
                                    {hasBaseRevision && (
                                      <>
                                        <div className="text-gray-500">Base Amount:</div>
                                        <div>
                                          <span className="line-through text-gray-400">₹{origBase.toLocaleString('en-IN')}</span>
                                          <span className="ml-1 font-semibold text-gray-900">→ ₹{baseAmt.toLocaleString('en-IN')}</span>
                                          <span className={`ml-1 text-[10px] ${baseAmt > origBase ? 'text-red-600' : 'text-emerald-600'}`}>
                                            ({baseAmt > origBase ? '+' : ''}{((baseAmt - origBase) / origBase * 100).toFixed(1)}%)
                                          </span>
                                        </div>
                                      </>
                                    )}
                                    {hasGstRevision && (
                                      <>
                                        <div className="text-gray-500">GST Amount:</div>
                                        <div>
                                          <span className="line-through text-gray-400">₹{origGst.toLocaleString('en-IN')}</span>
                                          <span className="ml-1 font-semibold text-gray-900">→ ₹{gst.toLocaleString('en-IN')}</span>
                                        </div>
                                      </>
                                    )}
                                  </div>
                              ...
                              <button
                                onClick={(e) => {
                                  ...
                                  if (invoice.status === 'approved') {
                                    ...
                                    setEditApprovalComment(`Tax invoice extension: ₹${extensionDelta.toLocaleString('en-IN')} (approved ₹${currentApproved.toLocaleString('en-IN')} → ₹${newTotal.toLocaleString('en-IN')})`);
                                    setEditApprovalError('');
                                  } else {
                                    setIncreaseAmountInvoice(invoice);
                                    setNewApprovedAmount(String(extensionDelta));
                                    setIncreaseComment(`Tax invoice extension: ₹${extensionDelta.toLocaleString('en-IN')}`);
                                  }
                                }}
                        ...
                        {/* Amendment log — show [AMENDED]/[RECALLED] history entries for submitted invoices */}
                        {isPending && approvalHistoryCache[invoice.id] && approvalHistoryCache[invoice.id].some(e => e.comments?.includes('[AMENDED]') || e.comments?.includes('[RECALLED]')) && (
                          <div className="bg-amber-50 rounded-lg p-3 border border-amber-200 mb-3">
                            <p className="text-xs font-semibold text-amber-700 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                              ...
                              Invoice Amended by Engineer
                            </p>
                            <div className="space-y-1.5">
                              {approvalHistoryCache[invoice.id]
                                .filter(e => e.comments?.includes('[AMENDED]') || e.comments?.includes('[RECALLED]'))
                                .map((entry) => (
                                  <div key={entry.id} className="text-xs text-amber-800 flex items-start gap-2">
                                    <span className="text-amber-400 flex-shrink-0 mt-0.5">•</span>
                                    <span>
                                      {entry.comments?.replace(/^\[(AMENDED|RECALLED)\]\s*/, '')}
                                      <span className="text-amber-500 ml-1">({entry.createdAt})</span>
                                    </span>
                                  </div>
                                ))}
                            </div>
